...
export default function MultiStepForm({ productData }) {
  ...
  useEffect(() => {
    ...
    const { slug, category, pesquisaType } = productData;
    ...
    // ✅ REGISTRO CIVIL
    if (category === 'Cartório de Registro Civil' && estadoAbbr) {
      basePrice = getPrice(
    ...
    // ✅ NOTAS
    if (category === 'Tabelionato de Notas (Escrituras)' && estadoAbbr) {
      basePrice = getPrice(
        'tabelionato_registro_civil',
        'estado',
        estadoAbbr,
    ...
    // ✅ IMÓVEIS
    if (category === 'Cartório de Registro de Imóveis' && estadoAbbr) {
      basePrice = getPrice(
        'registro_imoveis_pesquisas',
        'estado',
        estadoAbbr,
    ...
    if (slug === toSlug('Certidão de Protesto')) {
      ...
      if (
        todos_cartorios_protesto &&
        protestoCartorios[estado_cartorio]?.[cidade_cartorio]
      ) {
        multiplicador =
          protestoCartorios[estado_cartorio][cidade_cartorio].length;
      }
    }
    ...
    if (tipo_certidao === 'Condomínio') {
      total += getTaxa('acrescimo_condominio');
    }
